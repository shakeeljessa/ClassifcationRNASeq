...
for(i in getCSV(opt$target_matrix)) {
  ...
  predictions <- predict(fit.svmRadial, newdata = testing)
  ...
  #write out files
  #write.csv(fit.svmRadial[["results"]][["Kappa"]], file = generateFileDir("Kappas", opt$output_file_name, "csv"))
  #write.csv(output, file = generateFileDir("SVM/Predictions", opt$output_file_name, "csv"))
  #write.csv(cfByClass, file = generateFileDir("SVM/cfByClass", opt$output_file_name, "csv"))
  #write.csv(cfOverall, file = generateFileDir("SVM/cfOverall", opt$output_file_name, "csv"))
  #capture.output(cf, file = generateFileDir("SVM/ConfusionMatrices", opt$output_file_name, "csv"))
  #capture.output(fit.svmRadial, file = generateFileDir("SVM/fits", opt$output_file_name, "txt"))
  
  saveRDS(fit.svmRadial, file = generateFileDir(paste(opt$output_file_name,"Models/",last_dir,sep = ""), outputName,"rds"))
  ...
  capture.output(fit.svmRadial, file = generateFileDir(paste(opt$output_file_name,"Fits/",last_dir,sep =""), outputName, "txt"))
